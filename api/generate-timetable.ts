...
// Vercel-like request/response interfaces
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
    ...
    try {
        const { state, generationProfile, additionalConstraints } = req.body;
        const { classrooms, subjects, studentGroups, faculty, leaveRequests } = state as AppState;
...
`;
        ...
        res.status(200).json(generatedData as TimetableEntry[]);
